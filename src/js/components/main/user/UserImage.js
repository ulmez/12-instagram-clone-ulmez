...
import { authAndGetSpecUserImages, getSpecImageAndUser } from '../../../helpers/restGet';
        ...
        return (
            <div>
                ...
                <div className='user-image-box'>
                    <div>
                        <div className="right-place-link"><Link to='/user'>User</Link></div>
                        <h3 className="center-text">{this.state.user.name} Image List</h3>
                        <div>
                            {this.state.images.map((image) => (
                                <img key={image._id} id={image._id} src={image.url} onClick={this.onClickImage} className="thumbnail" />
                            ))}
                        </div>
                    </div>
                </div>
...
export default withRouter(connect(null, mapActionsToProps)(UserImage));